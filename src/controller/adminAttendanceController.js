...
export const clockIn = async (req, res) => {
  try {
    ...
    console.log("🔵 Admin Clock-In Request Received:", {
      adminId,
      companyId,
...
export const getAllAdminAttendance = async (req, res) => {
  try {
    const { date, adminId, startDate, endDate } = req.query;
    let filter = {};

    // Filter by specific date (today's attendance)
    if (date) {
    ...
    // Filter by date range (monthly attendance)
...
export const editAdminAttendance = async (req, res) => {
 ...
 /* -------------------------------------------------------------------------- */
/* CREATE ADMIN ATTENDANCE RECORD (FOR SUPER ADMIN MANUAL CREATE) */
 /* -------------------------------------------------------------------------- */

 export const createAdminAttendance = async (req,res) =>{
  try{
   
    const {admin,date,clockIn,clockOut,status}=req.body;

    if(!admin || !date){
      return res.status(400).json({message:"Admin and date are required"})
    ...
    // create admin attendence record
    ...
    await newAttendance.populate("admin", "fullName email")

    
    return res.status(201).json(
      { 
        message:"Admin attendance record created successfully",
        attendance:newAttendance
      });

  }
     catch (error){
    console.error("Error Creating admin attendance record", error);
    return res.status(500).json({
      message: "Server error while creating admin attendance",
      error: error.message
    });
  }
