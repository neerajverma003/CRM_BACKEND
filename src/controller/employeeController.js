...
export const AddEmployee = async (req, res) => {
  try {
    ...
    // 2️ Employee-specific required fields
    if (role === "Employee") {
      if (!company) {
        return res.status(400).json({
          success: false,
    ...
    // 3️ Admin role → these values become NULL (optional)
    const companyValue =
...
export const applyLeave = async (req, res) => {
  try {
    const { employeeId,appliedByModel, companyId, leaveType, startDate, endDate, reason } = req.body;
...
export const getMyLeaves = async (req, res) => {
  try {
    ...
    const leaves = await Leave.find({ employeeId })
      .populate("companyId", "companyName") //  Correct field name
      .sort({ appliedAt: -1 });
